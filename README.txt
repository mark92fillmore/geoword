An unfinished  standalone app that is composed of files primarily 
intended for use with PLOM.IO

There are funcitons for initializing the database based on basic files 
(which are included), searching the database for geo.json data
associated with human-recognizable names, and querying for data 
corresponding to areas contained in larger areas. 

There are many TODO statements that need attention (enumerated in the 
comments of each file). 

Data has been scraped from Natural Earth and Mike Bostock. 
